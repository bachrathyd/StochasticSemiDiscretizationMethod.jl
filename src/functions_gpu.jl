...
# ============================================================
# V2 Kernels  —  Modification A + B
#   A: Parallel init/extract (all threads, not tid==1 serial)
#   B: Strided loop for cross/diag  (no modulo-branch overhead)
# ============================================================
...
function kernel_M2_MF_v2!(
    ...
    @inbounds begin
        ...
        stride = Int32(blockDim().x)
        rp1    = r + Int32(1)
        L      = d * d
        total_C     = rp1 * rp1 * L   # elements in the full C buffer
        total_cross = rp1 * L          # cross-term elements per step
        ...
        for n in Int32(0):p - Int32(1)
            ...
            # Cross-terms C(n+1, m) for m in (n-r):n
            ...
            # Write-back cross-terms + symmetric copy
...
struct MFGPUMappingOperator{WsT} <: AbstractMatrix{Float64}
    coeffs::MFGPUCoefficients
    D::Int; r::Int; d::Int; p::Int
    ws::WsT
end
Base.size(op::MFGPUMappingOperator) = (op.D, op.D)
Base.size(op::MFGPUMappingOperator, i::Int) = i <= 2 ? op.D : 1
Base.eltype(op::MFGPUMappingOperator) = Float64
LinearAlgebra.issymmetric(op::MFGPUMappingOperator) = false
LinearAlgebra.ishermitian(op::MFGPUMappingOperator) = false

function LinearAlgebra.mul!(y::AbstractVector, op::MFGPUMappingOperator, x::AbstractVector)
    @cuda threads=256 blocks=1 kernel_M2_MF!(
        y, op.ws.v_in_zero, x, op.ws.v_in_zero,
        op.ws.C, op.ws.v, op.ws.C_next_m, op.ws.v_next,
        ...
        op.coeffs.stochV, op.coeffs.stochGV, op.coeffs.stochGV_k, op.coeffs.num_stochGV,
        Int32(op.p), Int32(op.r), Int32(op.d), false, op.ws.idx_sectionStarts
    )
    return y
end

# V2 operator — uses parallel init/extract + strided loops (Mods A+B)
struct MFGPUMappingOperator_v2{WsT} <: AbstractMatrix{Float64}
    coeffs::MFGPUCoefficients
    D::Int; r::Int; d::Int; p::Int
    ws::WsT
end
Base.size(op::MFGPUMappingOperator_v2) = (op.D, op.D)
Base.size(op::MFGPUMappingOperator_v2, i::Int) = i <= 2 ? op.D : 1
Base.eltype(op::MFGPUMappingOperator_v2) = Float64
LinearAlgebra.issymmetric(op::MFGPUMappingOperator_v2) = false
LinearAlgebra.ishermitian(op::MFGPUMappingOperator_v2) = false

function LinearAlgebra.mul!(y::AbstractVector, op::MFGPUMappingOperator_v2, x::AbstractVector)
    @cuda threads=256 blocks=1 kernel_M2_MF_v2!(
        y, x,
...
function spectralRadiusOfMapping_GPU(dm::stDiscreteMapping_M2_MF; solver=:KrylovKit, args...)
    ...
    op  = MFGPUMappingOperator(gpu_coeffs, D, r, d, p, ws)
    C0  = CUDA.rand(Float64, D)

    vals, _, _ = eigsolve(op, C0, 1, :LM; ishermitian=false, args...)
    return abs(vals[1])
end

function spectralRadiusOfMapping_GPU_v2(dm::stDiscreteMapping_M2_MF; args...)
    d = size(dm.coeffs.det[1][1][1], 1)
    p = dm.rst.n_steps
    r = div(dm.rst.n, d) - 1
    D = StochasticSemiDiscretizationMethod.CovVecIdx((r+1)*d).sectionStarts[end]
    ...
    gpu_coeffs = extract_gpu_coeffs(dm.coeffs, p, d, false)
    ws  = MFGPUWorkspace(dm.rst)
    op  = MFGPUMappingOperator_v2(gpu_coeffs, D, r, d, p, ws)
    C0  = CUDA.rand(Float64, D)
...
function fixPointOfMapping_GPU(dm::stDiscreteMapping_M2_MF; rtol=1e-15, args...)
    ...
    op2 = MFGPUMappingOperator(gpu_coeffs, D2, r, d, p, ws)
...
function spectralRadiusOfMapping_GPU_v3(dm::stDiscreteMapping_M2_MF;
    ...
    # Adaptive krylovdim: cap based on available GPU memory so the Krylov
    # basis (krylovdim vectors of length D) does not cause OOM.
    kd = if krylovdim > 0
        krylovdim
    else
        avail    = Int(CUDA.available_memory())
        reserved = Int(C0 |> sizeof) + 64 * 1024^2   # state vec + 64 MB headroom
        mem_kd   = max(4, (avail - reserved) ÷ max(1, D * 8))
        dim_kd   = max(4, D ÷ 500)   # D=253→4, D=5253→10, D≥15000→30
        min(30, min(mem_kd, dim_kd))
    end

    vals, _, _ = eigsolve(op, C0, 1, :LM; ishermitian=false, krylovdim=kd, args...)
    return abs(vals[1])
end
...
function spectralRadiusOfMapping_GPU_v4(dm::stDiscreteMapping_M2_MF;
                                         krylovdim::Int=0, args...)
    d = size(dm.coeffs.det[1][1][1], 1)
    p = dm.rst.n_steps
    r = div(dm.rst.n, d) - 1
    ...
    ws     = MFGPUWorkspace(dm.rst)
    n_sm   = Int(CUDA.attribute(CUDA.device(), CUDA.DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT))
    op     = MFGPUMappingOperator_v4(gpu_coeffs, D, r, d, p, n_sm, ws)
    CUDA.seed!(42)
    C0     = CUDA.rand(Float64, D)
    ...
    kd = if krylovdim > 0
        ...
        avail    = Int(CUDA.available_memory())
        reserved = Int(C0 |> sizeof) + 64 * 1024^2
        mem_kd   = max(4, (avail - reserved) ÷ max(1, D * 8))
        dim_kd   = max(4, D ÷ 500)
        min(30, min(mem_kd, dim_kd))
    end
...
function spectralRadiusOfMapping_GPU_v4g(dm::stDiscreteMapping_M2_MF;
                                          krylovdim::Int=0, args...)
    d = size(dm.coeffs.det[1][1][1], 1)
    p = dm.rst.n_steps
    r = div(dm.rst.n, d) - 1
    ...
    # Pre-warm all v4 kernels (required before graph capture to avoid JIT inside capture)
...
# Automatic dispatch: CPU-MF for small D, GPU-v3 (cooperative) for large D.
# Benchmarks show v3 (cooperative single-launch) is fastest on all tested p;
# v4g (CUDA-graph multi-launch) is ~5-10% slower due to sequential kernel overhead.
# cpu_threshold (~44 000) ≈ D at p=130 for d=2 on a Quadro P4000.
function spectralRadiusOfMapping_auto(dm::stDiscreteMapping_M2_MF;
                                       cpu_threshold::Int = 44_000,
                                       krylovdim::Int     = 0,
                                       args...)
    d = size(dm.coeffs.det[1][1][1], 1)
    r = div(dm.rst.n, d) - 1
    ...
    if D < cpu_threshold
        ...
        return spectralRadiusOfMapping_GPU_v3(dm; krylovdim=krylovdim, args...)
    end
end
